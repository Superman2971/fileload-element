...
export class FileComponent implements OnDestroy {
  ...
  @Output()
  public newFileDrop: EventEmitter<UploadEvent> = new EventEmitter<UploadEvent>();
  @Output()
  ...
  public onDragLeave(event: Event): void {
    if (!this.globalDisable && !this.disableIf) {
      if (this.dragoverflag) {
        ...
        this.newFileLeave.emit(event);
      }
      this.preventAndStop(event);
    }
  }

  filesInput(event: any) {
    // check if file from Input or from Drop
    ...
    if (!this.globalDisable && !this.disableIf) {
      ...
      this.subscription = timerObservable.subscribe(t => {
        if (this.stack.length === 0) {
          this.newFileDrop.emit(new UploadEvent(this.files));
          this.files = [];
